...
exports.listCoverLetters = async (req, res, next) => {
  try {
    ...
    const [letters, total] = await Promise.all([
      CoverLetter.find({ userId: req.user.userId }).select('-__v').sort({ createdAt: -1 }).skip(skip).limit(limit),
      CoverLetter.countDocuments({ userId: req.user.userId })
    ]);
    res.json({ success: true, message: "Cover letters listed", data: letters, meta: { total, page, totalPages: Math.ceil(total / limit) } });
  } catch (err) { next(err); }
};
...
exports.createCoverLetter = async (req, res, next) => {
  try {
    ...
    await letter.save();
    console.log(`[Cover Letter Created] User ${req.user.email} - Letter for job ${jobId}`);
    res.status(201).json({ success: true, message: "Cover letter created", data: { id: letter._id, ...letter.toObject() } });
  } catch (err) { next(err); }
};
...
exports.updateCoverLetter = async (req, res, next) => {
  try {
    ...
    if (!letter) return res.status(404).json({ success: false, message: "Cover letter not found" });
    console.log(`[Cover Letter Updated] User ${req.user.email} - Letter ${letter._id}`);
...
exports.deleteCoverLetter = async (req, res, next) => {
  try {
    const letter = await CoverLetter.findOneAndDelete({ _id: req.params.id, userId: req.user.userId });
    if (!letter) return res.status(404).json({ success: false, message: "Cover letter not found" });
    console.log(`[Cover Letter Deleted] User ${req.user.email} - Letter ${letter._id}`);
