...
exports.listJobMatches = async (req, res, next) => {
  try {
    ...
    const matches = await JobMatch.find(filters)
      .populate({ path: 'jobId', select: 'title company location postedDate jobType' })
...
exports.refreshJobMatches = async (req, res, next) => {
  try {
    ...
    // Archive old matches
    await JobMatch.updateMany({ userId: user._id, isActive: true }, { $set: { isActive: false, archivedAt: new Date() } });
    ...
    const summary = {
      ...
      averageScore: Math.round(created.reduce((s, j) => s + j.score, 0) / created.length)
    };
    res.json({
      success: true,
    ...
    console.log(`[JobMatch REFRESHED] User ${user.email}: ${created.length} matches`);
  } catch (err) { next(err); }
};
