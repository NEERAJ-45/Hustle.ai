...
const getDashboardData = async (userId) => {
  ...
  const [appAgg, matchAgg, user, rawUpcomingInterviews, rawMatches, rawApps] =
    await Promise.all([
      ...
      Application.find({
        ...
        .lean(),
      JobMatch.find({ userId: objectId, isActive: true })
  ...
  const appCounts = { total: 0, submitted: 0, interviews: 0, offers: 0 };
  ...
  const matchCounts = { total: 0, applied: 0, saved: 0 };
  for (const row of matchAgg) {
    matchCounts.total += row.count;
    const status = normalizeStatus(row._id);
    if (status === "applied") matchCounts.applied += row.count;
    if (status === "saved") matchCounts.saved += row.count;
  }
  ...
  const upcomingInterviews = rawUpcomingInterviews
    .map((application) => {
      ...
      return {
        ...
        location:
          nextInterview.location || application.jobId?.location?.city || "TBD",
      };
    })
    .filter(Boolean);
